...
DB는 Supabase(Postgres), 마이그레이션은 `supabase/migrations/`에 이 프로젝트가 직접 소유한다 (프론트엔드 `etfy`은 읽기 전용 소비자).

- `etf` — 쓰기는 이 프로젝트(secret key)만, 읽기는 `etfy`이 publishable key로 직접 SELECT (RLS `public_read_etf` 정책). PK는 `short_code`, `standard_code`에 unique 제약. `base_asset_class`/`base_market_class`에 인덱스 — 프론트엔드 필터 쿼리(`etf.api.ts`의 `.eq()`)가 이 컬럼들을 사용하기 때문.
- `kis_token_cache` — 싱글턴 테이블(`id=1` 고정), RLS 활성화 + 정책 없음(= service_role 역할만 접근 가능 — secret key로 인증, 완전 차단이 기본값). 이 테이블은 정규화 대상이 아니라 캐시이므로 일반적인 정규화 판단(중복 제거 등)을 적용하지 않는다.
