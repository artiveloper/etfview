...
- `etf` — 쓰기는 이 프로젝트(secret key)만, 읽기는 `etfview.kr`이 publishable key로 직접 SELECT (RLS `public_read_etf` 정책). PK는 `short_code`, `standard_code`에 unique 제약. `base_asset_class`/`base_market_class`에 인덱스 — 프론트엔드 필터 쿼리(`etf.api.ts`의 `.eq()`)가 이 컬럼들을 사용하기 때문.
