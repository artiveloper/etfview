...
description: >
  ...
  supabase-py 클라이언트, secret key, RLS, 마이그레이션, 싱글턴 캐시 테이블 관련
...
> 원칙: DB는 항상 RLS로 보호한다. secret key는 RLS를 우회하므로 서버 프로세스 밖으로 절대 나가지 않는다.

이 프로젝트는 사용자 로그인이 없는 **서버 전용 배치 수집기**다 — 원본(GamePot) `supabase-guide`가 다루는 브라우저/SSR 클라이언트, `getUser()`/`getSession()` 인증 패턴, JWT `app_metadata` 롤 체크, 미들웨어 토큰 갱신은 이 프로젝트에 존재하지 않는다. 대신 **단일 secret key 클라이언트로 모든 쓰기를 수행**하는 훨씬 단순한 모델을 쓴다.

> **API 키 이름 vs Postgres 역할명 — 혼동 주의**
> Supabase는 레거시 `anon`/`service_role` 키(JWT)를 `publishable`/`secret` 키로 교체했다 — 대시보드에서 발급받는 자격증명의 이름이 바뀐 것이다. 하지만 PostgREST가 요청을 매핑하는 **Postgres 역할명(`anon`/`authenticated`/`service_role`)은 그대로 유지된다.** RLS 정책의 `to anon, authenticated`나 "service_role만 접근 가능" 같은 표현은 역할명이지 키 이름이 아니므로, 새 키 체계로 바뀌어도 SQL을 고칠 필요가 없다. 이 문서에서 "secret key"/"publishable key"는 자격증명(코드·환경변수에 넣는 값)을, `anon`/`authenticated`/`service_role`은 SQL 안의 Postgres 역할명을 가리킨다 — 서로 다른 층이다.
...
| 키 | 용도 | 노출 |
|----|------|------|
| `SUPABASE_SECRET_KEY` | 유일한 클라이언트, RLS 우회(Postgres `service_role` 역할에 매핑) | 절대 노출 금지 — `.env`/컨테이너 시크릿으로만 |
...
> public 스키마의 모든 테이블에 RLS를 활성화한다. 예외 없음 — secret key는 RLS를 우회하므로 이 프로젝트의 쓰기 경로는 영향받지 않지만, RLS가 꺼진 테이블은 publishable/authenticated 자격증명을 가진 누구나 접근 가능해진다는 뜻이다.
...
-- INSERT/UPDATE/DELETE 정책 없음 — 쓰기는 이 프로젝트의 secret key만
```

**완전 비공개 테이블 (`kis_token_cache`, `job_execution_log`)** — secret key만 접근, 정책 자체를 만들지 않는다:

```sql
alter table public.kis_token_cache enable row level security;
-- 정책 없음 = anon/authenticated 역할 완전 차단, service_role 역할(secret key)만 접근 가능
```
...
- 이 프로젝트는 CLI가 로컬에 아직 설치되어 있지 않다(`0001_create_etf.sql` 등은 CLI 없이 작성됨) — CLI 설치 전까지는 Supabase Studio SQL Editor에 마이그레이션 SQL을 그대로 붙여넣어 적용하고, 적용한 파일은 그대로 `supabase/migrations/`에 남겨 이력을 유지한다.
...
- [ ] 공개 읽기 테이블은 `to anon, authenticated` + `using (true)`로 SELECT만 허용, 쓰기 정책은 만들지 않음(secret key 전용)
- [ ] 비공개 테이블(캐시 등)은 정책 자체를 만들지 않음(완전 차단이 기본값)
- [ ] `SUPABASE_SECRET_KEY`는 서버 전용, git/로그 노출 없음
